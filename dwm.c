 ...
 * increase capacity to 1024 to make space for formatting */
...
static void (*handler[LASTEvent]) (XEvent *) = {
	/* map xevent types to dwm functions */
...
static Clr **scheme;
static Clr **normtagscheme;
static Clr **seltagscheme;
static Display *dpy;
static Drw *drw;
...
cleanup(void)
{
	 ...
	 * increase number of color schemes to free up by +1 */
	...
	if (ev->window == root) {
		...
		if (updategeom() || dirty) {
			...
			for (m = mons; m; m = m->next) {
				...
				/* This resizes and repositions the bar according to the new position and size of the monitor. */
...
drawstatusbar(Monitor *m, int bh, char* stext) {
	...
	/* necessary to get status2d to play nice with statusbarpadding 
	 * https://www.reddit.com/r/dwm/comments/pbym3g/bar_padding_issue_in_the_dwm/ */
	w += 2*sp + 2; /* 1px padding on both sides */
	ret = x = m->ww - w;
...
drawbar(Monitor *m)
{
	...
	/* indicate that we start drawing from the beginning of the bar*/
	x = 0;
	for (i = 0; i < LENGTH(tags); i++) {
		w = TEXTW(tags[i]);

		/* Here we set the colour scheme to use when drawing the tag text. The gist of it is
		 * that we use seltagscheme[i] if the tag is being viewed and normtagscheme[i] otherwise. */
		drw_setscheme(drw, (m->tagset[m->seltags] & 1 << i ? seltagscheme[i] : normtagscheme[i]));
		...
		/* If the current tag is occupied by clients then draw an indicator. */
		if (occ & 1 << i)
			drw_rect(drw, x + boxw, 0, w - ( 2 * boxw + 1), boxw - 4,
	...
	if ((w = m->ww - tw - x) > bh) {
		/* removed show focused client's window title. */
		drw_setscheme(drw, scheme[SchemeNorm]);
...
{
	XEvent ev;
	/* main event loop - dpy is short for display */
	XSync(dpy, False);
	/* while wm is running find the next event in the queue */
	while (running && !XNextEvent(dpy, &ev))
		/* if we have a handler for that event type, pass it along */
		if (handler[ev.type])
...
setup(void)
{
	...
	lrpad = drw->fonts->h;
	bh = drw->fonts->h + bar_height_padding;
	updategeom();
	sp = sidepad;
	...
	/* init appearance */
	/* initialise colour schemes, and allocate memory to hold pointers to all colour schemes. 
	 * increase number of entries in colors array by 1 */
	scheme = ecalloc(LENGTH(colors) + 1, sizeof(Clr *));  /* initialise colour schemes, and allocate memory to hold pointers to all colour schemes. */
	/* then create the colour scheme for each entry. the last argument 3 represents the
	 * number of colours in each colour scheme array (foreground, background, and border colours). */
	scheme[LENGTH(colors)] = drw_scm_create(drw, colors[0], 3);
	for (i = 0; i < LENGTH(colors); i++)
		scheme[i] = drw_scm_create(drw, colors[i], 3);
	
	/* do a quick sanity check before proceeding */
	if (LENGTH(tags) > LENGTH(tagnorm) || LENGTH(tags) > LENGTH(tagsel))
		die("too few tag colors for number of tags defined");
	/* initialise tagschemes, and allocate memory to hold pointers to all tag colours. */
	normtagscheme = ecalloc(LENGTH(tagnorm), sizeof(Clr *));
	seltagscheme = ecalloc(LENGTH(tagsel), sizeof(Clr *));
	/* loop through all the entries in the tag colors array 
	 * then create the colour scheme for each entry. 
	 * the last argument 4 represents the number of colours 
	 * in each colour scheme array. */
	for (i = 0; i < LENGTH(tagnorm); i++)
		normtagscheme[i] = drw_scm_create(drw, tagnorm[i], 2);
	for (i = 0; i< LENGTH(tagsel); i++)
		seltagscheme[i] = drw_scm_create(drw, tagsel[i], 2);
...
{	
	...
	scan();
	restoreSession();
	/* this call will make dwm enter the event loop and it will remain there until the window manager is ready to exit. */
	run();
	cleanup();
	XCloseDisplay(dpy);
	return EXIT_SUCCESS;
